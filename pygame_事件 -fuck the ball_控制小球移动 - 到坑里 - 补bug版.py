					...
					for ball in group:
						if ball.control:
							...
							if not hole:
								...
								pygame.time.delay(5000)#延迟3秒
								victory = pygame.image.load(victory_image).convert_alpha()
								
								victory_position = (width - victory.get_width())//2,\
															(height - victory.get_height())//2
...
if __name__ == '__main__':
	#双击python文件，把错误代码显示出来，原本会直接退出python
	try:
		main()
	except SystemExit:#如果是删除xx出现的异常，就不会检测出来
		pass
	except:
		traceback.print_exc()
		pygame.quit()
